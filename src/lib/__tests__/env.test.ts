...
describe('env', () => {
  ...
  describe('checkEnvSetup', () => {
    it('should return isSetup true when API key is valid', () => {
      process.env.OPENAI_API_KEY = 'sk-1234567890abcdef';
      
      // env 객체를 다시 로드하기 위해 모듈을 리셋
      vi.resetModules();
      const { checkEnvSetup } = require('../../config/env');
    ...
    it('should return warning for placeholder API key', () => {
      process.env.OPENAI_API_KEY = 'your_openai_api_key_here';
    ...
    it('should return warning for invalid API key format', () => {
      process.env.OPENAI_API_KEY = 'invalid-key-format';
